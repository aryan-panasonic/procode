...
export default function PlatformClient() {
  ...
  const validIds = ["recognition","compliance","ocr","analytics","api","auto"];
  useEffect(() => {
    const activate = () => {
      const hash = window.location.hash.replace("#", "");
      if (validIds.includes(hash)) setActive(hash);
    };
    activate();
  ...
  return (
    <div className={styles.page}>
      ...
      <div className="container" style={{paddingBottom:"5rem"}}>
        <section id="capabilities" className={styles.challengeSection}>
          <h2 className={styles.challengeTitle}>{t("challengeTitle")}</h2>
          <p className={styles.challengeDesc}>{t("challengeDesc")}</p>
          <div className={styles.challengePoints}>
            {(t.raw("challengePoints") as string[]).map((p,i) => (
              <div key={i} className={styles.challengePoint}>
        ...
        <section id="modules" className={styles.tabsSection}>
        <div className={styles.tabBar}>
          {tabs.map(tItem=>(
            <button key={tItem.id} onClick={()=>handleTabClick(tItem.id)}
        ...
        <div id={active} className={styles.tabContent} style={{scrollMarginTop:"80px"}}>
        ...
        <section id="security" className={styles.securitySection}>
          <h2 className={styles.secTitle}>{t("secTitle")}</h2>
          <p className={styles.secDesc}>{t("secDesc")}</p>
          <div className={styles.secPoints}>
            {(t.raw("secPoints") as {icon:string,t:string,d:string}[]).map((p,i) => (
              <div key={i} className={styles.secPoint}>
                <span className={styles.secIcon}>{p.icon}</span>
                <div>
                  <div className={styles.secPointTitle}>{p.t}</div>
                  <div className={styles.secPointDesc}>{p.d}</div>
                </div>
              </div>
            ))}
          </div>
